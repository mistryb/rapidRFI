<?php

/**
 * This is the model class for table "tbl_timeline".
 *
 * The followings are the available columns in table 'tbl_timeline':
 * @property integer $rfi_id
 * @property string $date_received
 * @property string $date_entered
 * @property string $date_assigned
 * @property string $date_answered
 * @property string $date_closed
 * @property string $date_responded
 *
 * The followings are the available model relations:
 * @property Rfi $rfi
 */
class Timeline extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return Timeline the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'tbl_timeline';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('rfi_id, date_received, date_entered, date_assigned, date_answered, date_closed, date_responded', 'required'),
			array('rfi_id', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('rfi_id, date_received, date_entered, date_assigned, date_answered, date_closed, date_responded', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
			'rfi' => array(self::BELONGS_TO, 'Rfi', 'rfi_id'),
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'rfi_id' => 'Rfi',
			'date_received' => 'Date Received',
			'date_entered' => 'Date Entered',
			'date_assigned' => 'Date Assigned',
			'date_answered' => 'Date Answered',
			'date_closed' => 'Date Closed',
			'date_responded' => 'Date Responded',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('rfi_id',$this->rfi_id);
		$criteria->compare('date_received',$this->date_received,true);
		$criteria->compare('date_entered',$this->date_entered,true);
		$criteria->compare('date_assigned',$this->date_assigned,true);
		$criteria->compare('date_answered',$this->date_answered,true);
		$criteria->compare('date_closed',$this->date_closed,true);
		$criteria->compare('date_responded',$this->date_responded,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
}